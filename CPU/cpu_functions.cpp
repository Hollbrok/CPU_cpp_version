...
auto CPU(Bytecode* byte_struct, Stack* stk, Stack* stk_call) -> void
{
    ...
    else for (int i = 1; !FIND_HLT; i++)
    {
        ...
        {
			byte_struct->error_state += cmd_push_exe(command, i, stk, &rix_struct, byte_struct, OP);
			i++;
        }

        else if(get_byte(command, BIT_POP))	//(command == CMD_POP) //POP
        {
			byte_struct->error_state += cmd_pop_exe(command, i, stk, &rix_struct, byte_struct, OP);
            i++;
        }
        ...
        else
        {
            switch(command)
            {
                ...
                case CMD_MUL:
                    ...
                    {
                        printf("in mul\n");
                        stk->push(stk->pop() * stk->pop());// push_stack(stk, pop_stack(stk) * pop_stack(stk));
                    }
                    break;
                case CMD_DIV:
                {
				...
				case CMD_JMP:
				{
					byte_struct->error_state += cmd_compair_exe(command, stk, byte_struct, &i);
					break;
				}
	...
	determine_status(byte_struct, stk);
    free(OP);
    OP = nullptr;
}
...
auto draw_circ(char* OP, Stack* stk, Bytecode* byte_struct) -> void
{
	system("cls");
    ...
    else if (!get_byte(command, BIT_D_OP) && !get_byte(command, BIT_C_OP))
	{
		...
		else if (!get_byte(command, BIT_NUMBER))
		{
			...
			if (get_byte(rix_number, BIT_RAX))
			{
				...
				rix_struct->rax = stk->pop();//pop_stack(Stack);
				//printf("rax after = [%lg]\n", rix_struct->rax);
			}
			else if (get_byte(rix_number, BIT_RBX))
